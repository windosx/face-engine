...
// 如果调用初始化函数失败则返回一个错误
func NewFaceEngine(
	detectMode uint32, // 检测模式
	orientPriority uint32, // 检测角度
...
func (engine *FaceEngine) FaceFeatureExtractEx(
	...
	asfFaceInfo := &C.ASF_SingleFaceInfo{
		...
		C.MInt32(faceInfo.FaceOrient),
		faceInfo.DataInfo}
	r := C.ASFFaceFeatureExtractEx(engine.handle, imageDataToASVLOFFSCREEN(imageData), asfFaceInfo, uint32(registerOrNot), C.MInt32(mask), asfFaceFeature)
	if r != C.MOK {
		return FaceFeature{}, newError(int(r), "提取人脸特征失败")
	}
	length := int32(asfFaceFeature.featureSize)
